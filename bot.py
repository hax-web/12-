...
MORNING_CHANNEL_ID = 1538002650438565938  # 아침 7시 메시지를 보낼 채널
NIGHT_CHANNEL_ID = 1538002568914010192    # 밤 9시 메시지를 보낼 채널
...
async def check_time():
    ...
    if now.hour == 7 and now.minute == 0 and sent_today["morning"] != today:
        channel = client.get_channel(MORNING_CHANNEL_ID)
        if channel:
    ...
    if now.hour == 21 and now.minute == 0 and sent_today["night"] != today:
        channel = client.get_channel(NIGHT_CHANNEL_ID)
        if channel:
